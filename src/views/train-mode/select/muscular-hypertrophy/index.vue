 ...
 * @LastEditTime: 2022-12-23 10:35:52
 * @Description : 肌肥大训练-参数设置
-->
<template>
  <div class="muscular-hypertrophy-set">
    <div class="left">
      <!-- 介绍说明 -->
      <div class="introduce">
        <div class="item">训练部位：{{ trainPosition.split('-')[1] }}</div>
        <div class="item">
          训练推荐：最大力量的70%，重复10次，训练3组，每组休息30秒。
        </div>
      </div>
      <!-- 示意图 -->
...
export default {
  ...
  data() {
    return {
      ...
      entadRate: 1, // 向心比（1~10）
      keepdRate: 0, // 等长比（0~10）
      offcenterRate: 1 // 离心比（1~10）
    }
  },
          ...
          if (len) {
            if (res[len - 1].resultValue[trainPositionEn]) {
              this.target = parseInt(
                (res[len - 1].resultValue[trainPositionEn] * 0.7).toFixed(0)
              )
              // 训练目标不能低于10kg
              if (this.target < 10) {
                this.target = 10
    ...
    handleStart() {
      this.$router.push({
        ...
        query: {
          ...
          routerName: JSON.stringify('/train-select/muscular-hypertrophy-set')
        }
      })
    }
  }
